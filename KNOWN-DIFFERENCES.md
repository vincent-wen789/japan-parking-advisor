...
| **Claude** (Sonnet, web search on) | Yes — ran 8 real searches | resolves place names / addresses fine | plain Google Maps search links | Yes (in 中文) | Live test (川越, Alphard ~1.95m): derived dims from the model name, excluded all mechanical/tower lots, flagged 2.0m-capped lots as only ~5cm margin (don't trust blind), surfaced a free heads-up lot, 4 parallel alternatives. |
...
- **Claude** (2026-05-31, real live web-search test) — ran PROMPT.md against "川越 蔵造りの町並み, Toyota Alphard". Did **8 real web searches**, derived the Alphard's ~1.95m/1.85m/5.0m from the model name, excluded all mechanical/tower lots, flagged the common 2.0m-capped flat lots as only ~5cm margin (don't trust blind), surfaced a free 205-car heads-up lot, returned 4 parallel alternatives with the downgrade notice, answered in Chinese with JP lot names. **PASS.** (Earlier an offline 3-scenario behavioral test also passed: no-car → asked; 2.3m → excluded all; SUV → fit/conditional/excluded split.)
...
2. **Does it ask what car you drive before recommending?** (It should ask for the model, then derive the dimensions.) This is the safety-critical step; a model that skips it and silently assumes is a problem.
