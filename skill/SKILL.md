...
**Prefer to ask** the user for their car's **height and width** (length if relevant) when they haven't said it. **If you proceed without a confirmed car, assume a larger SUV (~1.65m H / 1.85m W), state that assumption, and invite a one-line correction — never silently assume a small or low car** (that's the only dangerous direction: it green-lights a lot a taller car can't physically enter). Offer quick presets: compact (~1.5m H / 1.7m W), sedan (~1.5m H / 1.8m W), SUV (~1.65m H / 1.85m W), minivan (~1.85m H / 1.85m W). The **height limit is a fixed fact of the lot** (it does not change with the car) — so collection is per-lot, filtering is per-car. **Height vs the lot's height limit is the safety-critical check** — a mechanical lot limited to 1.55m rejects most SUVs.
